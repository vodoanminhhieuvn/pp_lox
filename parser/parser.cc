...
#include <errors_and_debug/debug_print.h>
      ...
      if (match(TokenType::LESS)) {
        ...
        if (match(TokenType::IDENTIFIER)) {
          ...
          if ((std::get<AST::VariableExprPtr>(superClassVar)
                   ->varName.getLexeme() == className.getLexeme()))
...
auto RDParser::blockStmt() -> StmtPtrVariant {
  ...
  while (!match(TokenType::RIGHT_BRACE) && !isAtEnd()) {
    ...
    if (optStmnt.has_value())
      statements.push_back(std::move(optStmnt.value()));
  }
...
auto RDParser::call() -> ExprPtrVariant {
  ...
  while (true) {
    if (match(TokenType::LEFT_PAREN)) {
      ...
      expr =
          createCallEPV(std::move(expr), getTokenAndAdvance(), std::move(args));
...
auto RDParser::primary() -> ExprPtrVariant {
  if (match(TokenType::LOX_FALSE))
    return consumeOneLiteral("false");
  if (match(TokenType::LOX_TRUE))
    return consumeOneLiteral("true");
  if (match(TokenType::NIL))
    return consumeOneLiteral("nil");
  if (match(TokenType::NUMBER))
    return consumeOneLiteral();
  if (match(TokenType::STRING))
    return consumeOneLiteral();
  if (match(TokenType::LEFT_PAREN))
    return consumeGroupingExpr();
  if (match(TokenType::THIS))
    return AST::createThisEPV(getTokenAndAdvance());
  if (match(TokenType::IDENTIFIER))
    return consumeVarExpr();
